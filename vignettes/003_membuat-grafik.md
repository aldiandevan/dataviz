...
gdp_id %>% 
  filter(province %in% c("Banten", "DKI Jakarta", "Jawa Barat", "Jawa Tengah", "Di Yogyakarta", "Jawa Timur")) %>% # ekstrak provinsi di Pulau Jawa
  ggplot(aes(x = trade_hotel_and_restaurant, y = manufacturing, colour = province, size = 1)) +
  geom_point(alpha = 0.5)
```
...
) #memiringkan teks ben ora numpuk
```
...
``` r
ggplot(jabar_road, aes(x = district, y = road_length, fill = condition)) +
  geom_col(position = "fill") + theme(axis.text = element_text(angle = 30))
```
...
``` r
highest_gdp %>% 
  ggplot(aes(x = year , y = province, colour = year)) +
...
```
    ...
    ## Reading layer `states_provinces' from data source `C:\Users\Students\Documents\R Tisa\dataviz\data-raw\states_provinces.json' using driver `GeoJSON'
